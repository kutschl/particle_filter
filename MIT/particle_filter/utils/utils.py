...
# TODO convert this into numpy vectorized
class ParticleUtils:
    '''
    Utility class to convert particles [x,y,theta] into Pose objects.
    '''

    def __init__(self, laser_base_link_offset:np.ndarray):
        self.laser_base_link_offset = laser_base_link_offset

    def particle_to_pose(self, particle)->Pose:
        ''' Converts a particle in the form [x, y, theta] into a Pose object '''
        pose = Pose()
        pose.position.x = particle[0]
        pose.position.y = particle[1]
        pose.orientation = angle_to_quaternion(particle[2])
        pose = self.transpose_laser_to_base_link_xy(pose)

        return pose

    def transpose_laser_to_base_link_xy(self, pose):

        yaw = quaternion_to_angle(pose.orientation)
        pose.position.x = pose.position.x - \
            self.laser_base_link_offset[0] * math.cos(yaw)
        pose.position.y = pose.position.y - \
            self.laser_base_link_offset[0] * math.sin(yaw)
        return pose
    ...
    def particles_to_poses(self, particles):
        ...
        '''
        return list(map(self.particle_to_pose, particles))
